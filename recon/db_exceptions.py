...
def select_exceptions(server, database, username, password,Bank_code):
    ...
    excep_select_query = f"""
        SELECT DISTINCT DATE_TIME, TRAN_DATE, TRN_REF, BATCH, ACQUIRER_CODE, ISSUER_CODE, EXCEP_FLAG,
            CASE WHEN ACQ_FLG = 1 OR ISS_FLG =1 THEN 'Partly Receonciled' 
                 WHEN ACQ_FLG = 1 AND ISS_FLG =1 THEN 'Fully Reconciled' 
                 END AS RECON_STATUS
